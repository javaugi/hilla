...
export default class MyEntityModel<T extends MyEntity = MyEntity> extends MyEntityIdModel<T> {
  ...
  get decimalMax(): NumberModel {
    return this[_getPropertyModel]('decimalMax', NumberModel, [
      false,
      new DecimalMax({ value: '0.01', inclusive: false }),
    ]);
  }
  ...
  get entityMatrix(): ArrayModel<
    ModelValue<ArrayModel<ModelValue<MyEntityModel>, MyEntityModel>>,
    ArrayModel<ModelValue<MyEntityModel>, MyEntityModel>
  > {
  ...
  get complexMap(): ObjectModel<
    Record<
      string,
      ModelValue<ObjectModel<Record<string, ModelValue<ArrayModel<ModelValue<MyEntityModel>, MyEntityModel>>>>>
    >
  > {
    return this[_getPropertyModel]('complexMap', ObjectModel, [true]);
  }

  get nestedArrays(): ArrayModel<
    ModelValue<
      ArrayModel<
        ModelValue<ObjectModel<Record<string, ModelValue<ArrayModel<string, StringModel>>>>>,
        ObjectModel<Record<string, ModelValue<ArrayModel<string, StringModel>>>>
      >
    >,
    ArrayModel<
      ModelValue<ObjectModel<Record<string, ModelValue<ArrayModel<string, StringModel>>>>>,
